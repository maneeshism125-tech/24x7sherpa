...
from api import options_service
...
@router.get("/recommendations/options", response_model=OptionsRecommendationsResponse)
async def api_options_recommendations(refresh: bool = False) -> OptionsRecommendationsResponse:
    try:
        return await asyncio.to_thread(options_service.service_options_recommendations, refresh=refresh)
    except Exception as e:
        logger.exception("options recommendations failed")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/recommendations/options/{symbol}", response_model=OptionsTradeRecommendation)
async def api_options_symbol(symbol: str) -> OptionsTradeRecommendation:
    try:
        return await asyncio.to_thread(options_service.service_options_symbol, symbol=symbol)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except Exception as e:
        logger.exception("options symbol analysis failed")
        raise HTTPException(status_code=500, detail=str(e)) from e
